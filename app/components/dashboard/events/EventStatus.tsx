...
export default function EventStatus({ event }) {
  const title = titles[event.status][event.status === 'ACCEPTED' ? +(new Date() < new Date(event.subscriptions_end_at)) : 0]

  return (
    <Tooltip title={title}>
      <span>
        {event.status === 'WAITING_APPROVAL' && (
          <TimerSand className="text-yellow-500" />
        )}

        {event.status === 'ACCEPTED' && (new Date() < new Date(event.subscriptions_end_at) ? (
        ...
        {event.status === 'CHECKED_OUT' && (
          <CheckAll className="text-green-400" />
        )}
      </span>
    </Tooltip>
  )
