...
export default function SubscriptionRecap({ eventSubscriptionsQueryResult: [{ eventSubscriptions }, { isFetching }] }) {
    const getRecap = () => {
        ...
        eventSubscriptions.forEach((sub: EventSubscriptionWithTypedCart) => {
            sub.cart.forEach((item: CartItem) => {
                const exist = recap.find(exist => {
                    ...
                    const hasIdenticalOptions = exist.options.some(o => item.options.findIndex(y => y.name === o.name) !== -1)
