...
export default function Records({ leaderboard }) {
  const { data, isFetching }: { data: any; isFetching: boolean } = useCurrentUser({
          ...
          <TableBody>
            ...
            {data &&
              leaderboard.data.map((row) => {
                const score = data?.userStats?.articlesStats[row.articleId] ?? 0
                return (
                  <TableRow key={row.articleName}>
                    <TableCell align="right">
                      <Avatar src={row.articleImage} alt={`Image de ${row.articleName}`} />
                    </TableCell>
