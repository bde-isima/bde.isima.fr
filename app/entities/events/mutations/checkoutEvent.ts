...
export default resolver.pipe(resolver.authorize(['*', 'bde']), async ({ where }: UpdateEventInput) => {
    ...
    eventSubscriptions.map((s) => {
      if (s.payment_method === 'BDE') {
        ...
        const amount = (s as any).cart.reduce((acc: number, val: CartItem) => {
          return (
            acc +
            val.quantity * (val.price + (val.options?.reduce((acc: number, val: Option) => acc + val.price, 0) || 0))
          );
        }, 0);
