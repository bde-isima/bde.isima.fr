...
export default async function checkoutEvent({ where }: UpdateEventInput, ctx: Ctx) {
  ...
  const transactionsAndUsers = await Promise.all(eventSubscriptions.map(s => {
    if(s.payment_method === 'BDE') {
      ...
      const amount = (s as any).cart.reduce((acc: number, val: CartItem) => {
        return acc + val.quantity * (val.price + val.options.reduce((acc: number, val: Option) => acc + val.price, 0))
      }, 0)
